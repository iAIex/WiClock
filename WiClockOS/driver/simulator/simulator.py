...
class driver(object):
    ...
    def __init__(self, SharedInformation):
        ...
        self.px_x = self.SharedInformation.width
        self.px_y = self.SharedInformation.height
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RIGHT:
                    self.SharedInformation.threading_queue["ActionMgr"].put(["simulator", "RIGHT"])
                if event.key == pygame.K_LEFT:
                    self.SharedInformation.threading_queue["ActionMgr"].put(["simulator", "LEFT"])
                if event.key == pygame.K_SPACE:
                    self.SharedInformation.threading_queue["ActionMgr"].put(["simulator", "HOME"])
        ...
        for self.i in xrange(len(self.buttons)):
            if self.mxy[0] > self.buttons[self.i][0] and self.mxy[1] > self.buttons[self.i][1] and \
                ...
                if self.i == 1:
                    ...
                    self.id = "RIGHT"
                self.SharedInformation.threading_queue["ActionMgr"].put(["simulator", self.id])
